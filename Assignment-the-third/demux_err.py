...
def get_args():
	parser = argparse.ArgumentParser(description="Performs error correction on barcodes, then demultiplexes reads. Use for single-indexed libraries only.")
...
if args.min_qscore:  # minimum qscore cutoff default
...
for el in bar_dict:
    count_dict[el]=0
count_corrected=0 # keeps track of index-corrected read pairs
...
for i in range(numrecs):
    ...
    if bar1 not in bar_dict.values() or bar2 not in bar_dict.values():
        ## error correction on bar1 and bar2; returns original barcode if no correction possible.
        ## see Bioinfo.py module for details on error correction & hamming functions 
        bars = [b for b in bar_dict.values()]
        bar1 = Bioinfo.bc_correct(bar1, bars)
        bar2 = Bioinfo.bc_correct(bar1, bars)
        if bar1 in bar_dict.values() and bar2 in bar_dict.values() and bar1==bar2:
            count_corrected+=1
    ## create new fastq headers for output files
    new_head_r1 = r1_lst[0] + " " + bar1 + "-" + bar2 
    new_head_r2 = r2_lst[0] + " " + bar1 + "-" + bar2 
    ## if, after error correction, they still don't work, put in junk bins
    if bar1 not in bar_dict.values() or bar2 not in bar_dict.values():
...
for el in count_dict:
    print("Number of read pairs in", el, "bin:", count_dict[el])
print("Number of index-corrected read pairs:", count_corrected)
...
plt.savefig(dir_out+"/demux_err_hist.png", format="png")
...
with open(dir_out+"/bin_counts_err.csv", "a") as mdrpt:
    mdrpt.write(cmdline + "\n")
