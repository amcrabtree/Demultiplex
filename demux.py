...
def get_args():
	...
	parser.add_argument("-2", "--R2_file", help="I1 (forward index) file", required=True)
	parser.add_argument("-3", "--R3_file", help="I2 (reverse index) file", required=True) 
	parser.add_argument("-4", "--R4_file", help="R2 (reverse read) file", required=True)
...
for i in range(numrecs):
    ...
    if bar1 not in bar_dict.values() or bar2 not in bar_dict.values():
        ...
        for basepos in range(len(i1_lst[3])):  # check qscore of forward read index
            p=int(convert_phred(i1_lst[3][basepos]))
            if p < minq:
                q = 1
        for basepos in range(len(i2_lst[3])):  # check qscore of reverse read index
            p=int(convert_phred(i2_lst[3][basepos]))
            if p < minq:
                q = 1
